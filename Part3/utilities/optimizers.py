...
def mini_batch_gradient_descent(y, x, theta, max_iters, alpha, metric_type, mini_batch_size):
    ...
    for i in range(max_iters):
        ...
        # This is for MSE loss only
        for current_index in range(0, num_of_samples, mini_batch_size):
            batch_y = shuffled_y[current_index: current_index + mini_batch_size]
            batch_x = shuffled_x[current_index: current_index + mini_batch_size]

            gradient = -2 * (batch_x.T.dot(batch_y - batch_x.dot(theta))) / num_of_samples
            theta = theta - alpha * gradient
